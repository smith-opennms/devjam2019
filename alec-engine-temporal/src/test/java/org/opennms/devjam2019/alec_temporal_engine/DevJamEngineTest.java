...
package org.opennms.devjam2019.alec_temporal_engine;
...
import org.opennms.devjam.timeengine.DevJamEngineFactory;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public class DevJamEngineTest implements SituationHandler {

    private static final Logger LOG = LoggerFactory.getLogger(DevJamEngineTest.class);
    ...
    public void testFiveAlarmsSameTime() {
        Engine engine = new DevJamEngineFactory().createEngine();
        engine.registerSituationHandler(this);
        List<Alarm> alarms = getAlarms(5, 20);
        alarms.forEach(engine::onAlarmCreatedOrUpdated);
        // Terminate the test
        sendterminalAlarm(engine, 100);
    ...
    public void testTwoSituations() {
        ...
        engine.registerSituationHandler(this);
        // First Situation
        engine.onAlarmCreatedOrUpdated(ImmutableAlarm.newBuilder().setId("A").setTime(11000).build());
        engine.onAlarmCreatedOrUpdated(ImmutableAlarm.newBuilder().setId("B").setTime(12000).build());
        engine.onAlarmCreatedOrUpdated(ImmutableAlarm.newBuilder().setId("C").setTime(13000).build());
        // Second Situation
        engine.onAlarmCreatedOrUpdated(ImmutableAlarm.newBuilder().setId("X").setTime(31000).build());
        engine.onAlarmCreatedOrUpdated(ImmutableAlarm.newBuilder().setId("Y").setTime(32000).build());
